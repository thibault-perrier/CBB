...
public class ColorPicker : MonoBehaviour
{
    ...
    public void SetG(string value)
    {
        if (interact)
        {
            modifiedColor.g = (byte)Mathf.Clamp(int.Parse(value), 255, 255);
            RecalculateMenu(true);
        }
    }
    //gets b Slider value
    ...
    public void SetHexa(string value)
    {
        if (interact)
        {
            ...
            {
                modifiedColor = c;
                if (!useA) modifiedColor.a = 1; // Ensure alpha is set to 1 if not using alpha
                RecalculateMenu(true);
            }
            else
            {
    ...
    private sealed class HSV
    {
        ...
        public HSV() { }
        public HSV(double h, double s, double v, byte a = 255)
        {
            H = h;
            S = s;
            V = v;
            A = a; // Initialize alpha
        }
